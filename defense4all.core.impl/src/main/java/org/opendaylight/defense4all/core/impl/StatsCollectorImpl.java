...
//		removeStatsCounters(pnKey); // here is how to remove
	...
	public boolean setStatsCounters(String pnKey, List<String> newTrafficFloorLocs) throws ExceptionControlApp {
		...
		List<String> processedLocation = new ArrayList<String>();
		...
		while(iter.hasNext()) {
			...
			// Don't process same location twice in case we have few traffic floors
			if ( processedLocation.contains(oldTrafficFloorLoc ))
				continue;
			processedLocation.add(oldTrafficFloorLoc);
