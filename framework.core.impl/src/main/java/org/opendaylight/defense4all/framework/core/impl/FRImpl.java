...
public class FRImpl implements FR {
    ...
    public static class EventRecordImpl extends EventRecordData implements EventRecord {
        ...
        private static List<RepoCD> getEventRecordRCDs() {
            if (mEventsArchiveCDs == null) {
                ...
                rcd = new RepoCD(TIME_COUNTER, StringSerializer.get(), null);
                mEventsArchiveCDs.add(rcd);
                rcd = new RepoCD(EVENT_TIME, DateSerializer.get(), null);
                mEventsArchiveCDs.add(rcd);
                rcd = new RepoCD(EVENT_TYPE, StringSerializer.get(), null);
                mEventsArchiveCDs.add(rcd);
                rcd = new RepoCD(EVENT_DATA, StringSerializer.get(), null);
                mEventsArchiveCDs.add(rcd);
            }
    ...
    public static class FilterRecordImpl implements FilterRecord {
        ...
        public boolean match(EventRecord eventRecord) {
            ...
            if (-1 == matchId) return false; // no such events in the repo
     ...
     * Setters for Spring
     */
    public void setRepoFactoryImpl(RepoFactoryImpl repoFactoryImpl) {
        this.repoFactoryImpl = repoFactoryImpl;
    }

    public void setFlightRecorderSliceDays(int slice) {
        this.flightRecorderSliceDays = slice;
    }
    ...
    public void setOutputFileSuffix(String suffix) {
    ...
    public void init() throws ExceptionControlApp {
        ...
        try {
            eventsArchiveRepo = repoFactoryImpl.getOrCreateRepo(repoGlobal, RepoMinor.EVENTS.name(), StringSerializer.get(), true, EventRecordImpl.getEventRecordRCDs());
            timeSliceRepo = repoFactoryImpl.getOrCreateRepo(repoGlobal, RepoMinor.TIME_SLICES.name(), StringSerializer.get(), true, null);
        } catch (Exception e) {
    ...
    public synchronized void logRecord(EventRecord iEvent) {
        ...
        try {
            if (event.getKey() == null) event.generateKey();
            eventsArchiveRepo.setRow(event.getKey(), event.toRow());
            addToSlice(event);
        } catch (ExceptionControlApp e) {
    ...
    /**
     * @param fileName dump content of records to file
    ...
    public void dump(String fileName, FilterRecord filter) throws ExceptionControlApp {
        ...
        try {
            ...
            if (!file.exists()) file.createNewFile();    // if file doesnt exists, create it
            ...
            if (eventSlicesKeys == null) return;
    ...
    public List<EventRecordData> getTimeRangeEvents(Date fromDate, Date toDate, int maxNum, FilterRecord filter) {
        ...
        while (eventSlicesKeysIter.hasNext()) {
            ...
            if (!onInterval && Long.valueOf(eventSliceKey) < sliceFrom) continue;
            if (Long.valueOf(eventSliceKey) > sliceTo) break;
    ...
    public void reset(int days) {
        ...
        while (eventSlicesKeysIter.hasNext()) { // Go over slices to find latest slice
            ...
            String eventSliceKey = eventSlicesKeysIter.next();
            if (Long.valueOf(eventSliceKey) > latestSlice) break;
    ...
    public void testRepo() {
        try {
            ...
            if (latest == null) System.out.println("No events");
            else {
