...
public class PN {
	...
	public PN() {
		...
		ofBasedDetection = false; pnCanceled = false; numofAttackSuspicions = 0; 
	...
	public PN(Hashtable<String, Object> pnRow) throws UnknownHostException {
		...
		if (pnRow.get(NUMOF_ATTACK_SUSPICIONS)!=null)
			numofAttackSuspicions = (Integer) pnRow.get(NUMOF_ATTACK_SUSPICIONS);
	...
	public static List<RepoCD> getPNRCDs() {
		...
		if(mPNsRepoCDs == null) {
			...
			rcd = new RepoCD(NUMOF_ATTACK_SUSPICIONS, IntegerSerializer.get(), null); mPNsRepoCDs.add(rcd);
